...
export class ContractViewerComponent implements OnInit {
  ...
  onTransferClicked(contract: ContractAgreement) {
    ...
    dialogRef.afterClosed().pipe(first()).subscribe(result => {
      ...
      let trqObject: any; // Variable, um das trq-Objekt zu speichern
  
      this.createTransferRequest(contract, storageTypeId, baseUrl)
        .pipe(
          tap(trq => {
            trqObject = trq; // Das trq-Objekt in der Variablen speichern
            console.log('Transfer Request Object:', trqObject); // Output the trq object to console
          }),
          switchMap(trq => this.transferService.initiateTransfer(trq))
        )
        .subscribe(transferId => {
          this.startPolling(transferId, contract["@id"]!);
        }, error => {
  ...
  private createTransferRequest(contract: ContractAgreement, storageTypeId: string, baseUrl: string): Observable<TransferProcessInput> {
    return this.getContractOfferForAssetId(contract.assetId!).pipe(map(contractOffer => {
      ...
      if (storageTypeId === "HttpData") {
        //dataDestination['@type'] = "DataAddress",
        dataDestination.type = "HttpData",
        dataDestination.properties = {
          baseUrl: baseUrl
        }
      }
      if (storageTypeId === 'AzureStorage') {
        dataDestination.account = this.homeConnectorStorageAccount;
      ...
      const initiateTransfer : TransferProcessInput = {
